...
void main() {
    ...
    if (screenPxRange < 2.5)
    {
        ...
        opacity = (clamp(screenPxRange * (sampleMsdf(TexCoord + dx + dy) - 0.5) + 0.5, 0.0, 1.0) +
        clamp(screenPxRange * (sampleMsdf(TexCoord - dx + dy) - 0.5) + 0.5, 0.0, 1.0) +
        clamp(screenPxRange * (sampleMsdf(TexCoord + dx - dy) - 0.5) + 0.5, 0.0, 1.0) +
        clamp(screenPxRange * (sampleMsdf(TexCoord - dx - dy) - 0.5) + 0.5, 0.0, 1.0)) * 0.25;
    } else
    {
